...
{
  struct real r ;
  r = multiply_fixed_point_and_integer(thread_current()-> recent_cpu,100);
  return  convert_fixed_to_integer_nearest(r);
}
...
schedule (void) 
{
  ...
  if(cur->status == THREAD_RUNNING)
    list_insert_ordered (&ready_list, &cur,priority_compare,NULL);
...
void update_priority(struct thread *t){
   ...
   if(t != idle_thread)
   {
      ...
      int second_term = t->nice * 2;
      struct real third_term = subtract_fixed_point_and_integer_right(PRI_MAX,first_term);
      struct real result = subtract_fixed_point_and_integer(third_term,second_term);
...
void update_load_avg(){
  ...
  struct real term = convert_integer_to_fixed_point(ready_size);
...
void update_recent_cpu(struct thread *t){
    ...
    struct real result = add_fixed_point_and_integer(fourth_term,t->nice);
    t->recent_cpu = result;
}
